...
# Параметры сети:
variable "vpc_id" {
  description = "VPC ID для создания ВМ"
  type        = string
}

variable "zone" {
  description = "Зона доступности для создания ВМ"
  type        = string
}
...
variable "platform_id" {
  ...
  validation {
    ...
    error_message = "Platform ID должна быть одна из: standard-v1, standard-v2, standard-v3."
  }
}
...
variable "core_fraction" {
  description = "Уровень производительности vCPU в процентах (5, 25, 50, 100)"
  type        = number
  default     = 100
...
variable "disk_type" {
  ...
  validation {
    ...
    error_message = "Тип диска должен быть один из: network-hdd, network-ssd, network-ssd-nonreplicated."
  }
}
